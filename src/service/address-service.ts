...
export class AddressService {
  ...
  static async create(user: User, request: CreateAddressRequest): Promise<AddressResponse> {
    ...
    await ContactService.checkContactMustBeExists(user.username, request.contact_id)
  ...
  static async CheckAddressMustBeExists(contactId: number, addressId: number): Promise<Address>{
  ...
  static async get(user: User, request: GetAddressRequest): Promise<AddressResponse> {
    ...
    const address = await this.CheckAddressMustBeExists(getRequest.contact_id, getRequest.id)
  ...
  static async update(user: User, request: UpdateAddressRequest): Promise<AddressResponse>{
    ...
    await ContactService.checkContactMustBeExists(user.username, request.contact_id)
    await this.CheckAddressMustBeExists(updateRequest.contact_id, updateRequest.id)
  ...
  static async remove(user: User, request: RemoveAddressRequest): Promise<AddressResponse> {
    ...
    await ContactService.checkContactMustBeExists(user.username, request.contact_id)
    await this.CheckAddressMustBeExists(removeRequest.contact_id, removeRequest.id)
  ...
  static async list(user: User, contactId: number): Promise<Array<AddressResponse>> {
    await ContactService.checkContactMustBeExists(user.username, contactId)
